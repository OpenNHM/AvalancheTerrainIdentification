...
import ati.mod3Map.regionalThalwegAnalysis as regionalThalwegAnalysis
...
def thalwegAnalysisMain(avaDir=None, cfgThalwegAnalysis=None):
    ...
    cfgThalwegAnalysis: configparser object
        setup
    """
    ...
    if cfgThalwegAnalysis is None:
        cfgThalwegAnalysis = cfgUtils.getModuleConfig(runThalwegAnalysis)

    if cfgThalwegAnalysis["GENERAL"].getboolean("runFlowPy"):
        ...
        praDelineationCfg = cfgUtils.getModuleConfig(
            ...
            modInfo=False,
            toPrint=False,
            onlyDefault=cfgThalwegAnalysis["mod1Release_praDelineationVeitinger_override"].getboolean("defaultConfig"),
        )
        praDelineationCfg, _ = cfgHandling.applyCfgOverride(
            praDelineationCfg, cfgThalwegAnalysis, praDelineationVeitinger, addModValues=False
        )
        ...
        inRelJsonDir = avaDir / "Inputs" / "RELJSON"
        ...
        mod1ReleaseCfg = cfgUtils.getModuleConfig(
            ...
            modInfo=False,
            toPrint=False,
            onlyDefault=cfgThalwegAnalysis["mod1Release_mod1Release_override"].getboolean("defaultConfig"),
        )
        ...
        inRelJsonDir.mkdir(exist_ok=True)
        ...
        if len(list(inRelJsonDir.glob("*.geojson"))) > 0:
            message = "In Inputs/RELJSON folder is a PRA file, the generated file is not copied!"
            log.info(message)
        else:
            praJsonFile = list(praPrepDir.glob("*-5.geojson"))
            if len(praJsonFile) == 0:
                message = f"No REL Vol file is in {praPrepDir} to copy into Inputs/RELJSON!"
                log.error(message)
                raise FileNotFoundError(message)
            elif len(praJsonFile) > 1:
                message = f"Too many REL Json files in {praPrepDir} to copy into Inputs/RELJSON!"
                log.error(message)
                raise FileNotFoundError(message)

            shutil.copy2(praJsonFile[0], inRelJsonDir)
        ...
        compParamsCfg = cfgUtils.getModuleConfig(
            ...
            modInfo=False,
            toPrint=False,
            onlyDefault=cfgThalwegAnalysis["mod2Mobility_compParams_override"].getboolean("defaultConfig"),
        )
        compParamsCfg, _ = cfgHandling.applyCfgOverride(
            compParamsCfg, cfgThalwegAnalysis, compParams, addModValues=False
        )
        ...
        FlowPyCfg = cfgUtils.getModuleConfig(
            ...
            modInfo=False,
            toPrint=False,
            onlyDefault=cfgThalwegAnalysis["com4FlowPy_com4FlowPy_override"].getboolean("defaultConfig"),
        )
        FlowPyCfg, _ = cfgHandling.applyCfgOverride(FlowPyCfg, cfgThalwegAnalysis, com4FlowPy, addModValues=False)
    ...
    regThalwegCfg = cfgUtils.getModuleConfig(
        regionalThalwegAnalysis,
        fileOverride="",
        modInfo=False,
        toPrint=False,
        onlyDefault=cfgThalwegAnalysis["mod3Map_regionalThalwegAnalysis_override"].getboolean(
            "defaultConfig"
        ),
    )
    regThalwegCfg, _ = cfgHandling.applyCfgOverride(
        regThalwegCfg, cfgThalwegAnalysis, regionalThalwegAnalysis, addModValues=False
    )
    regionalThalwegAnalysis.regionalThalweg2DPlotMain(avaDir, regThalwegCfg, simhash=FlowPyHash)
