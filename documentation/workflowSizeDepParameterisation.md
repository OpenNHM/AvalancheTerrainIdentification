# Workflow: Size dependent Parameterization

## Overview

The **size dependent parameterization workflow** (`../workflows/runSizeDepParameterisation.py`) combines the `mod1Release`
...
The workflow is implemented in `sizeDepParameterisationMain` (`../workflows/runSizeDepParameterisation.py`).
...
### 2. Size dependent Mobility Parameterization
...
- **`runSizeDepParameterisationCfg.ini`** (workflow settings): for each sub-module, an override section controls
...
base configuration, with the parameter values given in `runSizeDepParameterisationCfg.ini` applied on top as
overrides. If `False`, and a `local_` configuration file for that sub-module is available, that local
configuration is used instead.
 
As with `atiCfg.ini`, create a `local_runSizeDepParameterisationCfg.ini` copy to adjust these settings for your
own run.
...
pixi shell
python workflows/runSizeDepParameterisation.py
```
